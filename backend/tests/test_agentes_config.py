import pytest
from config.agentes_config import (
    AGENTES_CONFIG,
    get_agente_config,
...
class TestAgentesConfig:
    ...
    def test_todos_agentes_configurados(self):
        """Verificar que los 10 agentes están configurados"""
        agentes_esperados = [
        ...
        for agente in agentes_esperados:
            assert agente in AGENTES_CONFIG, f"Agente {agente} no encontrado"
    ...
    def test_candados_duros_en_pmo(self):
        ...
        assert config is not None, "A2_PMO no encontrado"
    ...
    def test_estrategia_tiene_vision_pilares(self):
        """A1_ESTRATEGIA debe tener documento de visión y pilares"""
        docs = get_documentos_rag("A1_ESTRATEGIA")
        assert "001_vision_pilares" in docs
        assert "002_planeacion_okrs" in docs
    
    def test_get_contexto_obligatorio_retorna_lista(self):
        """get_contexto_obligatorio debe retornar lista de strings"""
        obligatorios = get_contexto_obligatorio("A1_ESTRATEGIA")
        assert isinstance(obligatorios, list)
        assert all(isinstance(c, str) for c in obligatorios)
    
    def test_subagentes_configurados(self):
        """Los 3 subagentes deben estar configurados"""
        subagentes = ["S1_TIPIFICACION", "S2_MATERIALIDAD", "S3_RIESGOS"]
        for sub in subagentes:
            config = get_agente_config(sub)
            assert config is not None, f"Subagente {sub} no encontrado"
            assert config.rol is not None
