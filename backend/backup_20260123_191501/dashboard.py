...
async def get_projects(status: Optional[str] = None):
    """Get all projects filtered by current empresa"""
    empresa_id = get_current_empresa_id()
    
    try:
        from services.defense_file_service import defense_file_service
        from services.project_serializer import flatten_project_for_list
        ...
        for df in defense_files:
            ...
            flattened = flatten_project_for_list(df, full_df)
            
            if status and flattened["status"] != status.lower():
                continue
            
            flattened["status_display"] = PROJECT_STATUSES.get(flattened["status"].upper(), flattened["status"])
            formatted_projects.append(flattened)
        
        return {"projects": formatted_projects, "total": len(formatted_projects), "empresa_id": empresa_id}
    except Exception as e:
...
async def get_project(project_id: str):
    """Get project by ID, validating empresa ownership"""
    from services.defense_file_service import defense_file_service
    ...
    try:
        project = await db_service.get_project(project_id, empresa_id=empresa_id)
        ...
        if not project:
            ...
            if defense_file:
                ...
                flattened = flatten_project_for_frontend(defense_file, include_deliberations=False)
                
                return {
                    "project": flattened,
                    "defense_file": defense_file,
                    "interactions": defense_file.get("deliberations", [])
                }
            
            raise HTTPException(status_code=404, detail="Project not found")
        
        interactions = await db_service.get_interactions(project_id, empresa_id=empresa_id)
